...
export class DkOverlayBehavior extends LitElement {
  static get styles() {
    return css`
    :host {
      position: fixed;
  ...
  _boundScrollHandler(event) {
    ...
    let isInnerScroll = path.find((ele) => {
      return (ele.tagName && ele.hasAttribute('id') && ele.getAttribute('id') && ele.getAttribute('id') == 'scroller');
    });
      
    let insideScroll = path.find((itemEl) => {
      return itemEl === isInnerScroll;
    });
...
customElements.define('dk-overlay-behavior', DkOverlayBehavior);